...
from net.model import Net4_2Net as net4
from net.model import refineNet_2Net
...
def main():
    global opt, Net1 , Net2  , Net4 , RefineNet , criterion_mse , criterion_ssim_map,criterion_ssim,criterion_ace
    ...
    optimizer = [ 1 , optimizer1 , optimizer2  , optimizer4 , optimizer_Refine ]
    print("==========> Training")
    for epoch in range(opt.startepoch, opt.nEpoch + 1):
        ...
        if epoch > 10 :
            ...
            optimizer[3] = optim.Adam(filter(lambda p: p.requires_grad, Net4.parameters()), lr=opt.lr)
            optimizer[4] = optim.Adam(filter(lambda p: p.requires_grad, RefineNet.parameters()), lr=opt.lr)

        train(training_data_loader, optimizer, epoch)

        if epoch % opt.test_frequency == 0 and epoch >10:
...
def train(training_data_loader, optimizer, epoch):
    ...
    for step, (data, label) in enumerate(training_data_loader, 1):
        ...
        Net4_out = Net4( data - Net1_out  ,data - Net2_out )
        RefineNet_out =RefineNet( Net1_out , Net2_out  , data - Net4_out )
        ...
        loss = new_loss + 0.01 * (loss1 + loss2 )


        del Net1_out , Net2_out  , Net4_out
        Net1.zero_grad()
        Net2.zero_grad()
        # Net3.zero_grad()
        Net4.zero_grad()
        ...
        if step % opt.train_print_fre == 0:
            print("epoch{} step {} loss {:6f}  new_loss {:6f} ssimloss {:6f}  loss1 {:6f}  loss2 {:6f} ".format(epoch, step,
                                                                                               loss.item(),
                                                                                               new_loss.item(),
                                                                                               ssim_loss.item(),
                                                                                               loss1.item(),
...
def test(test_data_loader, epoch):
    ...
    with torch.no_grad():
        ...
        for test_step, (data, label, data_path) in enumerate(test_data_loader, 1):
            ...
            Net4_out = Net4(data - Net1_out , data - Net2_out ).cuda() #best rain streaks
            refineNet_out = RefineNet(Net1_out , Net2_out  , data - Net4_out ).cuda()

            del Net1_out, Net2_out,
