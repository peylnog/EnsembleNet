...
class refineNet(nn.Module):
    '''refinenet: input 512 512 12 ,
        3 rainstreaks with background info
        1 derain image
        out put :512 512 3  derain '''

    def __init__(self):
        super(refineNet,self).__init__()

        self.up_dim = deconv_blocks_size_3(15 , 24) #1024 1024 24
        self.down_dim1 = conv_blocks_size_3(24, 15,True )

        self.reduce1 = conv_block_size_3(15,9)

        self.reduce2 = conv_block_size_3(9, 6)

        self.reduce3 = conv_block_size_3(6, 3)



    def forward(self, x1 , x2 , x3 , x4 ,data ):
        x = self.up_dim(torch.cat([x1 , x2 , x3 , x4 , data ] , dim = 1)) #15

        x = self.down_dim1(x)
        return self.reduce3(self.reduce2(self.reduce1(x)))
